...
void draw_game_power_ups(const vector<power_up_data> &power_ups_to_draw)
{
    ...
    {
        draw_power_up(power_ups_to_draw[i]);
        //write_line(std::to_string(center_point(game.power_ups[i].power_up_sprite).x) + ", " + std::to_string(center_point(game.power_ups[i].power_up_sprite).y));
    }
}
...
void update_power_up(power_up_data &power_up_to_update)
{
    ...
    double left_bound = camera_x() - 5 * SCREEN_BORDER;
    double right_bound = camera_x() + screen_width() + 5 * SCREEN_BORDER;
    double top_bound = camera_y() - 5 * SCREEN_BORDER;
    double bottom_bound = camera_y() + screen_height() + 5 * SCREEN_BORDER;
...
{
    draw_sprite(fuel_to_draw.fuel_sprite);
}
